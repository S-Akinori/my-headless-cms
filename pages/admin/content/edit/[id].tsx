import { addContent, addContentType, addFile, editContent, getContent, getContentType, getContentTypes } from "lib/fetchAPI";
...
const AdminContentEditPage = () => {
  ...
  useEffect(() => {
    if(id) {
      getContent(process.env.NEXT_PUBLIC_DOMAIN_ID as string, id as string).then((content) => {
        ...
        setContentObject(content.object);
        getContentType(process.env.NEXT_PUBLIC_DOMAIN_ID as string, content.typeId as string).then((contentType) => {
  ...
  const onChange : ChangeEventHandler<HTMLInputElement | HTMLTextAreaElement> = async (e) => {
    ...
    if(e.currentTarget.type === 'file') {
      ...
      if(key && Array.isArray(contentObject[parentName])) {
        ...
        console.log('new object', newContentObject)
  ...
  const saveContent: MouseEventHandler<HTMLButtonElement> = async (e) => {
    ...
    try {
      await editContent('MtDx5o2bav9hxzQxyshx', content);
      alert('保存しました')
              ...
              <div>
                {selectedContentType.items && selectedContentType.items.map(item => (
                  <div key={item.id} className="mb-6">
                    ...
                    {item.type === '一行テキスト' && <Input name={item.name} value={contentObject[item.name] as string} onChange={onChange} />}
                    {item.type === '複数行テキスト' && <TextArea rows={8} name={item.name} value={contentObject[item.name] as string} onChange={onChange} />}
                    {item.type === '画像' && (
                      <div>
                    ...
                    {item.type === 'フィールドセット' && item.items && <Fieldset fieldset={item} contentObjects={contentObject[item.name] as ContentObject[]} onChange={onChange} />}
                  </div>
                ))}
              </div>
            </div>
